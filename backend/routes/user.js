...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const profileImage = req.files["profile_image"] ? `/uploads/${req.files["profile_image"][0].filename}` : null;
      const vehicleImage = req.files["vehicle_image"] ? `/uploads/${req.files["vehicle_image"][0].filename}` : null;
