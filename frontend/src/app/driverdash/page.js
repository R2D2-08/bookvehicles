...
import { Car, User, ToggleLeft, ToggleRight, Bell, MapPin, Phone, UserCircle } from "lucide-react";
...
socket.on("newRideRequest", ({ rideDetails, passengerId }) => {
  console.log("New ride request:", rideDetails);

  // Accept or Reject ride
  const acceptRide = (accepted) => {
    socket.emit("respondToRide", { passengerId, driverId: socket.id, accepted });
  };

  // Example: Auto-accepting for testing
  acceptRide(true);
});

// Send driver location updates
const updateLocation = (location) => {
  socket.emit("updateLocation", { userId: socket.id, location });
};

// Listen for passenger location updates
socket.on("passengerLocation", (location) => {
  console.log("Passenger is here:", location);
});
...
const DriverDashboard = () => {
  ...
  const [rideRequests, setRideRequests] = useState([
    { id: 1, name: "John Doe", phone: "+123456789", pickup: [40.7128, -74.006], dropoff: "Airport", fare: "$25", time: "2 mins ago" },
    { id: 2, name: "Jane Smith", phone: "+987654321", pickup: [40.758, -73.9855], dropoff: "Train Station", fare: "$15", time: "5 mins ago" },
  ]);
  ...
  const notifyPassenger = () => {
    setRideOngoing(true); // Change button to "End Journey"
  };
  
  const endJourney = () => {
    setShowReviewModal(true); // Show rating/review modal
  };
  
  const submitReview = () => {
    setShowReviewModal(false);
    setActiveRide(null); // Clear active ride
    setRideOngoing(false); // Reset button
    setActiveTab("notifications"); // Go back to ride requests screen
  };
  ...
  return (
    <div className="flex h-screen bg-gray-50">
      ...
      <div className="w-1/5 bg-white shadow-xl p-6 flex flex-col gap-6 items-center border-r">
        ...
        <button
          className={`flex items-center gap-3 p-3 w-full rounded-xl transition-all ${
            activeTab === "car" ? "bg-blue-500 text-white" : "bg-gray-200 hover:bg-gray-300"
          }`}
        ...
        <button
          className={`flex items-center gap-3 p-3 w-full rounded-xl transition-all ${
            activeTab === "notifications" ? "bg-blue-500 text-white" : "bg-gray-200 hover:bg-gray-300"
          }`}
      ...
      <div className="flex-1 p-8">
        ...
        {activeTab === "car" && (
          <div className="flex flex-col justify-center items-center h-full">
            <Image src="/images/car.webp" alt="Driver's Car" width={500} height={300} className="rounded-xl shadow-lg" />
            <p className="mt-4 text-lg text-gray-700 text-center max-w-md font-semibold">
              The Ferrari R-800 is an amazing addition to the collection of cars you would want to transit in.
            </p>
          </div>
        )}
          ...
          <div className="flex flex-col gap-4 p-6 bg-white shadow-md rounded-xl max-w-lg mx-auto">
            ...
            {rideRequests.length > 0 ? (
              rideRequests.map((request) => (
                <div key={request.id} className="p-4 bg-gray-100 rounded-xl shadow flex flex-col gap-2">
                  <p className="text-gray-800 font-semibold">
                    <strong>Pickup:</strong> {request.pickup.join(", ")} → <strong>Drop-off:</strong> {request.dropoff}
                  </p>
                  <p className="text-gray-600 font-medium">Fare: {request.fare}</p>
              ...
              <p className="text-gray-500 text-lg font-semibold text-center">No new ride requests.</p>
            )}
          </div>
        )}
        ...
        {showReviewModal && (
          <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-30 backdrop-blur-sm">
            <div className="bg-white p-6 rounded-xl shadow-xl max-w-md w-full relative">
              ...
              <h2 className="text-2xl font-bold mb-4 text-center text-gray-800">Rate the Passenger</h2>
              ...
              <div className="flex justify-center gap-2 mb-4">
                {[1, 2, 3, 4, 5].map((star) => (
                  <button 
                    key={star} 
                    className={`text-3xl transition ${rating >= star ? "text-yellow-500" : "text-gray-300 hover:text-yellow-400"}`}
